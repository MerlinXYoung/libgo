...
void Processer::AddTask(Task *tk) {
  DebugPrint(dbg_task | dbg_scheduler, "task(%s) add into proc(%u)(%p)",
             tk->DebugInfo(), id_, (void *)this);
...
void Processer::AddTask(SList<Task> &&slist) {
  DebugPrint(dbg_scheduler, "task(num=%d) add into proc(%u)", (int)slist.size(),
             id_);
  std::unique_lock<TaskQueue::lock_t> lock(newQueue_.LockRef());
...
void Processer::NotifyCondition() {
  ...
  if (waiting_) {
    DebugPrint(
        dbg_scheduler,
        "NotifyCondition for condition. [Proc(%d)] --------------------------",
        id_);
    cv_.notify_all();
  } else {
    DebugPrint(
        dbg_scheduler,
        "NotifyCondition for flag. [Proc(%d)] --------------------------", id_);
    notified_ = true;
  }
}
...
#if ENABLE_DEBUGGER
    DebugPrint(dbg_scheduler,
               "Run [Proc(%d) QueueSize:%lu] --------------------------", id_,
               RunnableSize());
#endif
...
#if ENABLE_DEBUGGER
      DebugPrint(dbg_switch, "leave task(%s) state=%d",
                 runningTask_->DebugInfo(), (int)runningTask_->state_);
#endif
...
bool Processer::IsCoroutine() { return !!GetCurrentTask(); }

std::size_t Processer::RunnableSize() const noexcept {
...
void Processer::WaitCondition() {
  ...
  if (notified_) {
    DebugPrint(
        dbg_scheduler,
        "WaitCondition by Notified. [Proc(%d)] --------------------------",
        id_);
    notified_ = false;
    return;
  }
...
bool Processer::IsBlocking() const {
  if (!markSwitch_ || markSwitch_ != switchCount_)
    return false;
  return NowMicrosecond() >
         markTick_ + CoroutineOptions::getInstance().cycle_timeout_us;
}
...
int64_t Processer::NowMicrosecond() const {
  return std::chrono::duration_cast<std::chrono::microseconds>(
             FastSteadyClock::now().time_since_epoch())
      .count();
}
  ...
  if (n > 0) {
    ...
    if (runningTask_)
      pushRunningTask = runnableQueue_.eraseWithoutLock(runningTask_, true) ||
                        slist.erase(runningTask_, newQueue_.check_);
    if (nextTask_)
      pushNextTask = runnableQueue_.eraseWithoutLock(nextTask_, true) ||
                     slist.erase(nextTask_, newQueue_.check_);
    ...
    if (!slist2.empty())
      DebugPrint(dbg_scheduler, "Proc(%d).Stealed = %d", id_,
                 (int)slist2.size());
    return slist2;
  } else {
    // steal all
    ...
    slist2.append(std::move(slist));
    if (!slist2.empty())
      DebugPrint(dbg_scheduler, "Proc(%d).Stealed all = %d", id_,
                 (int)slist2.size());
    return slist2;
  }
}
...
Processer::SuspendEntry
Processer::Suspend(FastSteadyClock::time_point timepoint) {
...
Processer::SuspendEntry Processer::SuspendBySelf(Task *tk) {
  ...
  DebugPrint(dbg_suspend, "tk(%s) Suspend. nextTask(%s)", tk->DebugInfo(),
             nextTask_->DebugInfo());
...
bool Processer::IsExpire(SuspendEntry const &entry) {
  ...
  if (!tkPtr)
    return true;
  if (entry.id_ != TaskRefSuspendId(tkPtr.get()))
    return true;
  return false;
}
...
bool Processer::WakeupBySelf(IncursivePtr<Task> const &tkPtr, uint64_t id,
  ...
  DebugPrint(
      dbg_suspend,
      "tk(%s) Wakeup. tk->state_ = %s. is-in-proc(%d). sizeAfterPush=%lu",
      tk->DebugInfo(), GetTaskStateName(tk->state_),
      GetCurrentProcesser() == this, sizeAfterPush);
