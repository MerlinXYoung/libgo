...
class Processer {
  ...
  public:
    ALWAYS_INLINE int Id() const noexcept { return id_; }

    static Processer *&GetCurrentProcesser();

    static Scheduler *GetCurrentScheduler();

    inline Scheduler *GetScheduler() noexcept { return scheduler_; }
    ...
    struct SuspendEntry {
        ...
        friend bool operator<(SuspendEntry const &lhs, SuspendEntry const &rhs) {
            if (lhs.id_ == rhs.id_) return lhs.tk_ < rhs.tk_;
            return lhs.id_ < rhs.id_;
        }

        bool IsExpire() const { return Processer::IsExpire(*this); }
    };
  ...
  private:
    ...
    // 待执行的协程数量
    // 暂兼用于负载指数
    std::size_t RunnableSize() const noexcept;
    ...
    // 调度线程会尽量分配协程过来
    ALWAYS_INLINE bool IsWaiting() const noexcept { return waiting_; }
...
#if ENABLE_DEBUGGER
    DebugPrint(dbg_yield, "yield task(%s) state = %s", tk->DebugInfo(),
               GetTaskStateName(tk->state_));
    if (Listener::GetTaskListener()) Listener::GetTaskListener()->onSwapOut(tk->id_);
#endif
