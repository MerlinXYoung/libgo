...
bool &Scheduler::IsExiting() noexcept {
...
void Scheduler::CreateTask(TaskF const &fn, TaskOpt const &opt) {
    Task *tk = new Task(
        fn, opt.stack_size_ ? opt.stack_size_ : CoroutineOptions::getInstance().stack_size);
...
bool Scheduler::IsCoroutine() const { return !!Processer::GetCurrentTask(); }

bool Scheduler::IsEmpty() const noexcept { return taskCount_ == 0; }

void Scheduler::Start(int minThreadNumber, int maxThreadNumber) {
    if (!started_.try_lock()) throw std::logic_error("libgo repeated call Scheduler::Start");

    if (minThreadNumber < 1) minThreadNumber = std::thread::hardware_concurrency();
    ...
    if (maxThreadNumber_ > 1) {
        ...
        std::thread t([this] {
            DebugPrint(dbg_thread, "Start dispatcher(sched=%p) thread id: %lu", (void *)this,
                       NativeThreadID());
    ...
    DebugPrint(dbg_scheduler, "Scheduler::Start minThreadNumber_=%d, maxThreadNumber_=%d",
               minThreadNumber_, maxThreadNumber_);
    mainProc->Process();
}
...
void Scheduler::NewProcessThread() {
    ...
    std::thread t([this, p] {
        DebugPrint(dbg_thread, "Start process(sched=%p) thread id: %lu", (void *)this,
                   NativeThreadID());
        p->Process();
    });
    t.detach();
...
void Scheduler::LoadBalance(Scheduler::ActiveMap &actives, std::size_t activeTasks) {
    ...
    if (actives.begin()->first > avg * CoroutineOptions::getInstance().load_balance_rate) return;
...
void Scheduler::DispatcherThread() {
    ...
    while (!stop_) {
        ...
        std::this_thread::sleep_for(
            std::chrono::microseconds(CoroutineOptions::getInstance().dispatcher_thread_cycle_us));
...
void Scheduler::AddTask(Task *tk) {
    ...
    for (std::size_t i = 0; i < pcount; ++i, ++idx) {
        ...
        if (proc && proc->active_) break;
    }
    proc->AddTask(tk);
}

uint32_t Scheduler::TaskCount() const noexcept { return taskCount_; }

uint64_t Scheduler::GetCurrentTaskID() const noexcept {
    Task *tk = Processer::GetCurrentTask();
    return tk ? tk->id_ : 0;
}

uint64_t Scheduler::GetCurrentTaskYieldCount() const noexcept {
    Task *tk = Processer::GetCurrentTask();
