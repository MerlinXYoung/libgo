...
class RoutineSyncTimerT {
  public:
    ...
    virtual ~RoutineSyncTimerT() { stop(); }
    ...
    struct FuncWrapper {
        ...
        void cancel() { canceled_.store(false, std::memory_order_release); }
    ...
    void schedule(TimerId &id, const std::chrono::time_point<_Clock, _Duration> &abstime,
                  func_type const &fn) {
        ...
        while (!exit_) {
            ...
            if (id) {
                std::chrono::milliseconds delta =
                    std::chrono::duration_cast<std::chrono::milliseconds>(id->key - nowTp);
                sleepTime = (std::min)(sleepTime, delta);
            } else {
                sleepTime = loop_interval();
            }

            nextCheckAbstime_ = std::chrono::duration_cast<std::chrono::nanoseconds>(
                                    (now() + sleepTime).time_since_epoch())
                                    .count();
  ...
  private:
        ...
        if (&id == orderedList_.front()) {
            if (!front || (front && id.key < front->key)) {
                if (std::chrono::duration_cast<std::chrono::nanoseconds>(
                        (id.key).time_since_epoch())
                        .count() < nextCheckAbstime_) {
                    cv_.notify_one();
                }
            }
        }
    ...
    clock_type::time_point convert(const clock_type::time_point &abstime) { return abstime; }
