...
template <typename T, typename QueueT> class ChannelImpl;

template <typename T, typename QueueT = std::deque<T>> class Channel;

template <typename T>
class ChannelImplWithSignal : public DebuggerId<ChannelImplWithSignal<int>> {
public:
  template <typename _Clock, typename _Duration>
  bool pop_impl_with_signal_noqueued(
      T &t, bool isWait,
    ...
    RS_DBG(dbg_channel,
           ...
           id(), __func__, (void *)&t, isWait, !!abstime, closed_,
           (void *)popWaiting_, (void *)pushWaiting_);

    if (pushWaiting_ &&
        pushWaiting_ != (ConditionVariable *)
                            kClosedWaiting) { // 有push协程在等待, 读出来 & 清理
      t = *pushQ_;
      pushQ_ = nullptr;
    ...
    if (!isWait) {
      RS_DBG(dbg_channel,
             "channel=%ld | %s | not match && not wait | return false", id(),
             __func__);
      return false;
    }
    ...
    RS_DBG(dbg_channel, "channel=%ld | %s | begin wait matched", id(),
           __func__);

    (void)waiting.wait_until_p(lock, abstime,
                               [&] { return popWaiting_ != &waiting; });
    bool changed = (popWaiting_ != &waiting);
    bool ok = changed && popWaiting_ != (ConditionVariable *)kClosedWaiting;

    RS_DBG(dbg_channel,
           "channel=%ld | %s | waked | closed=%d | changed=%d | ok=%d", id(),
  ...
  bool pop_impl_with_signal(
    ...
    if (!isWait) {
      RS_DBG(dbg_channel,
             "channel=%ld | %s | pop contended && not wait | return false",
             id(), __func__);
      return false;
    }

    // 有其他pop在等待, 进pop队列
    RS_DBG(dbg_channel, "channel=%ld | %s | begin wait at pop queued", id(),
           __func__);
    ...
    RS_DBG(dbg_channel, "channel=%ld | %s | waked | closed=%d | pop idle", id(),
           __func__, closed_);
  ...
  template <typename _Clock, typename _Duration>
  bool push_impl_with_signal_noqueued(
      T const &t, bool isWait,
      const std::chrono::time_point<_Clock, _Duration> *abstime,
      std::unique_lock<Mutex> &lock) {
    RS_DBG(dbg_channel,
           "channel=%ld | %s | ptr(t)=0x%p | isWait=%d | abstime=%d | "
    ...
    if (popWaiting_ && popWaiting_ !=
                           (ConditionVariable *)
                               kClosedWaiting) { // 有pop协程在等待, 写入 & 清理
      *popQ_ = t;
      popQ_ = nullptr;
  ...
  bool push_impl_with_signal(
    ...
    if (!isWait) {
      RS_DBG(dbg_channel,
             "channel=%ld | %s | push contended && not wait | return false",
             id(), __func__);
      return false;
    }

    // 有其他push在等待, 进push队列
    RS_DBG(dbg_channel, "channel=%ld | %s | begin wait at push queued", id(),
           __func__);
...
private:
  ...
  bool push_impl_with_cap(
    ...
    RS_DBG(dbg_channel,
           ...
           id(), __func__, (void *)&t, isWait, !!abstime, closed_, cap_,
           q_.size());
    ...
    if (q_.size() < cap_) {
      ...
      RS_DBG(dbg_channel,
             "channel(queue)=%ld | %s | ptr(t)=0x%p | push success | notify",
             id(), __func__, (void *)&t);
    ...
    if (!isWait) {
      RS_DBG(dbg_channel,
             "channel(queue)=%ld | %s | not match && not wait | return false",
             id(), __func__);
      return false;
    }
    ...
    RS_DBG(dbg_channel,
           "channel(queue)=%ld | %s | waked | status=%s | closed=%d", id(),
           __func__,
  ...
  template <typename _Clock, typename _Duration>
  bool
  pop_impl_with_cap(T &t, bool isWait,
                    const std::chrono::time_point<_Clock, _Duration> *abstime) {
    std::unique_lock<Mutex> lock(mtx_);
    ...
    if (!q_.empty()) {
      ...
      RS_DBG(dbg_channel,
             "channel(queue)=%ld | %s | ptr(t)=0x%p | pop success | notify",
             id(), __func__, (void *)&t);
...
private:
  ...
  bool push_impl_with_cap(
    ...
    if (!isWait) {
      RS_DBG(dbg_channel,
             "channel(void)=%ld | %s | not match && not wait | return false",
             id(), __func__);
      return false;
    }
    ...
    RS_DBG(dbg_channel,
           "channel(void)=%ld | %s | waked | status=%s | closed=%d", id(),
           __func__,
           (status == std::cv_status::timeout) ? "timeout" : "no_timeout",
           closed_);
    ...
    RS_DBG(
        dbg_channel,
        "channel(void)=%ld | %s | push success after wait | size=%lu | notify",
        id(), __func__, count_);
  ...
  template <typename _Clock, typename _Duration>
  bool
  pop_impl_with_cap(nullptr_t &t, bool isWait,
                    const std::chrono::time_point<_Clock, _Duration> *abstime) {
    std::unique_lock<Mutex> lock(mtx_);
    ...
    if (count_ > 0) {
      ...
      RS_DBG(dbg_channel, "channel(void)=%ld | %s | pop success | notify", id(),
             __func__);
      return true;
    }
    ...
    RS_DBG(
        dbg_channel,
        "channel(void)=%ld | %s | pop success after wait | size=%lu | notify",
        id(), __func__, count_);
...
template <typename T, typename QueueT> class Channel {
...
public:
  ...
  Channel const &operator<<(T const &t) const {
    if (!impl_->push(t, true, RoutineSyncTimer::null_tp()) &&
        throw_ex_if_operator_failed_) {
  ...
  template <typename U>
  typename std::enable_if<!std::is_same<U, std::nullptr_t>::value &&
                              std::is_same<U, T>::value,
                          Channel const &>::type
  operator>>(U &t) const {
    if (!impl_->pop(t, true, RoutineSyncTimer::null_tp()) &&
        throw_ex_if_operator_failed_) {
      throw std::runtime_error("channel operator>>(T) error");
    }
    return *this;
  ...
  bool push_until(T const &t,
                  RoutineSyncTimer::clock_type::time_point deadline) const {
  ...
  template <typename Rep, typename Period>
  bool pop_for(std::nullptr_t ignore,
  ...
  bool pop_until(std::nullptr_t ignore,
                 RoutineSyncTimer::clock_type::time_point deadline) const {
    T t;
    return impl_->pop(t, true, &deadline);
  }
