...
class ChannelImplWithSignal : public DebuggerId<ChannelImplWithSignal<int>> {
  public:
    ...
    bool pop_impl_with_signal_noqueued(T &t, bool isWait,
        ...
        RS_DBG(dbg_channel,
               "channel=%ld | %s | ptr(t)=0x%p | isWait=%d | abstime=%d | "
               "closed=%d | popWaiting_=%p | pushWaiting_=%p",
               id(), __func__, (void *)&t, isWait, !!abstime, closed_, (void *)popWaiting_,
               (void *)pushWaiting_);

        if (pushWaiting_ &&
            pushWaiting_ !=
                (ConditionVariable *)kClosedWaiting) {  // 有push协程在等待, 读出来 & 清理
            t = *pushQ_;
            pushQ_ = nullptr;

            RS_DBG(dbg_channel, "channel=%ld | %s | match pop success | notify peer", id(),
                   __func__);
        ...
        if (!isWait) {
            RS_DBG(dbg_channel, "channel=%ld | %s | not match && not wait | return false", id(),
                   __func__);
            return false;
        }
        ...
        RS_DBG(dbg_channel, "channel=%ld | %s | begin wait matched", id(), __func__);
        ...
        RS_DBG(dbg_channel, "channel=%ld | %s | waked | closed=%d | changed=%d | ok=%d", id(),
               __func__, closed_, changed, ok);
    ...
    bool pop_impl_with_signal(T &t, bool isWait,
        ...
        if (!isWait) {
            RS_DBG(dbg_channel, "channel=%ld | %s | pop contended && not wait | return false", id(),
                   __func__);
            return false;
        }

        // 有其他pop在等待, 进pop队列
        RS_DBG(dbg_channel, "channel=%ld | %s | begin wait at pop queued", id(), __func__);

        auto p = [this] { return !popWaiting_; };
        while (!p()) {
            if (popCv_.wait_until_impl(lock, abstime) == std::cv_status::timeout) {
                RS_DBG(dbg_channel, "channel=%ld | %s | waked | pop busy | timeout", id(),
                       __func__);
                return false;
            }
        }

        RS_DBG(dbg_channel, "channel=%ld | %s | waked | closed=%d | pop idle", id(), __func__,
               closed_);
    ...
    bool push_impl_with_signal_noqueued(T const &t, bool isWait,
        ...
        if (popWaiting_ &&
            popWaiting_ != (ConditionVariable *)kClosedWaiting) {  // 有pop协程在等待, 写入 & 清理
            *popQ_ = t;
            popQ_ = nullptr;

            RS_DBG(dbg_channel, "channel=%ld | %s | match push success | notify peer", id(),
                   __func__);
    ...
    bool push_impl_with_signal(T const &t, bool isWait,
        ...
        // 有其他push在等待, 进push队列
        RS_DBG(dbg_channel, "channel=%ld | %s | begin wait at push queued", id(), __func__);

        auto p = [this] { return !pushQ_; };
        while (!p()) {
            if (pushCv_.wait_until_impl(lock, abstime) == std::cv_status::timeout) {
                RS_DBG(dbg_channel, "channel=%ld | %s | waked | push busy | timeout", id(),
                       __func__);
                return false;
            }
        }

        RS_DBG(dbg_channel, "channel=%ld | %s | waked | closed=%d | push idle", id(), __func__,
               closed_);
    ...
    void impl_with_signal_close(std::unique_lock<Mutex> &lock) {
        ...
        (void)pop_wakeup;
        RS_DBG(dbg_channel,
               "channel(...)=%ld | %s | no-cap branch | push-wakeup=%ld | "
               "pop-wakeup=%ld",
...
class ChannelImpl : public ChannelImplWithSignal<T> {
  public:
    ...
    explicit ChannelImpl(std::size_t capacity = 0) : cap_(capacity) {}

    template <typename _Clock, typename _Duration>
    bool push(T const &t, bool isWait, const std::chrono::time_point<_Clock, _Duration> *abstime) {
        if (cap_) {
    ...
    template <typename _Clock, typename _Duration>
    bool pop(T &t, bool isWait, const std::chrono::time_point<_Clock, _Duration> *abstime) {
        if (cap_) {
    ...
    void close() {
        ...
        RS_DBG(dbg_channel,
               "channel(queue)=%ld | %s | cap=%lu | size=%lu | push-wakeup=%ld | "
               "pop-wakeup=%ld",
  ...
  private:
    ...
    bool push_impl_with_cap(T const &t, bool isWait,
        ...
        RS_DBG(dbg_channel,
               "channel(queue)=%ld | %s | ptr(t)=0x%p | isWait=%d | abstime=%d | "
               "closed=%d | cap=%lu | size=%lu",
        ...
        RS_DBG(dbg_channel, "channel(queue)=%ld | %s | begin wait", id(), __func__);

        std::cv_status status = pushCv_.wait_until_p(lock, abstime, p);

        RS_DBG(dbg_channel, "channel(queue)=%ld | %s | waked | status=%s | closed=%d", id(),
               __func__, (status == std::cv_status::timeout) ? "timeout" : "no_timeout", closed_);
        ...
        RS_DBG(dbg_channel,
               "channel(queue)=%ld | %s | ptr(t)=0x%p | push success after wait | "
               "size=%lu | notify",
    ...
    bool pop_impl_with_cap(T &t, bool isWait,
        ...
        RS_DBG(dbg_channel,
               "channel(queue)=%ld | %s | ptr(t)=0x%p | pop success after wait | "
               "size=%lu | notify",
               id(), __func__, (void *)&t, q_.size());
...
class ChannelImpl<nullptr_t, QueueT> : public ChannelImplWithSignal<nullptr_t> {
  public:
    ...
    explicit ChannelImpl(std::size_t capacity = 0) : cap_(capacity), count_(0) {}
    ...
    template <typename _Clock, typename _Duration>
    bool pop(nullptr_t &t, bool isWait, const std::chrono::time_point<_Clock, _Duration> *abstime) {
        if (cap_) {
            return pop_impl_with_cap(t, isWait, abstime);
        }
    ...
    void close() {
        ...
        RS_DBG(dbg_channel,
               "channel(void)=%ld | %s | cap=%lu | size=%lu | push-wakeup=%ld | "
               "pop-wakeup=%ld",
  ...
  private:
    ...
    bool push_impl_with_cap(nullptr_t const &t, bool isWait,
        ...
        RS_DBG(dbg_channel,
               "channel(void)=%ld | %s | isWait=%d | abstime=%d | closed=%d | "
               "cap=%lu | size=%lu",
        ...
        if (count_ < cap_) {
            ...
            RS_DBG(dbg_channel, "channel(void)=%ld | %s | push success | notify", id(), __func__);
        ...
        RS_DBG(dbg_channel, "channel(void)=%ld | %s | begin wait", id(), __func__);

        std::cv_status status = pushCv_.wait_until_p(lock, abstime, p);

        RS_DBG(dbg_channel, "channel(void)=%ld | %s | waked | status=%s | closed=%d", id(),
               __func__, (status == std::cv_status::timeout) ? "timeout" : "no_timeout", closed_);
    ...
    bool pop_impl_with_cap(nullptr_t &t, bool isWait,
        ...
        if (count_ > 0) {
            ...
            RS_DBG(dbg_channel, "channel(void)=%ld | %s | pop success | notify", id(), __func__);
            return true;
        }
...
class Channel {
  ...
  public:
    explicit Channel(std::size_t capacity = 0, bool throw_ex_if_operator_failed = true) {
    ...
    template <typename U>
    typename std::enable_if<!std::is_same<U, std::nullptr_t>::value && std::is_same<U, T>::value,
                            Channel const &>::type
    operator>>(U &t) const {
    ...
    bool push(T const &t) const { return impl_->push(t, true, RoutineSyncTimer::null_tp()); }
    ...
    bool try_push(T const &t) const { return impl_->push(t, false, RoutineSyncTimer::null_tp()); }
    ...
    inline bool unique() const { return impl_.unique(); }

    inline void close() const { impl_->close(); }

    inline bool closed() const noexcept { return impl_->closed(); }

    inline bool empty() const noexcept { return impl_->empty(); }

    inline std::size_t size() const noexcept { return impl_->size(); }
...
class Channel<void, QueueT> : public Channel<std::nullptr_t, QueueT> {
  public:
    explicit Channel(std::size_t capacity = 0) : Channel<std::nullptr_t>(capacity) {}
};
