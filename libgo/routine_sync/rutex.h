...
struct RutexWaiter : public LinkedNode, public DebuggerId<RutexWaiter> {
  public:
    ...
    bool wake(int state = waiter_state_ready) {
        if (waked_.load(std::memory_order_relaxed)) return false;
    ...
    void wake_by_timer() {
        ...
        if (!waked_.load(std::memory_order_relaxed)) {
            RS_DBG(dbg_rutex, "rw=%ld | %s | wake failed, retry delay=%d", id(), __func__,
                   (int)delayMs_);
            // 可能还没来得及sleep, 往后等一等
            RoutineSyncTimer::getInstance().reschedule(
                timerId_, RoutineSyncTimer::now() + std::chrono::milliseconds(delayMs_));
            delayMs_ = delayMs_ << 1;
        } else {
            RS_DBG(dbg_rutex, "rw=%ld | %s | wake failed, needn't retry.", id(), __func__);
        }
    }
    ...
    inline int state() const noexcept { return state_; }
  ...
  public:
    ...
    rutex_wait_return wait_until(IntValueType expectValue,
        ...
        if (base_t::value()->load(std::memory_order_relaxed) != expectValue) {
            ...
            RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d | check fail | %s", id(), __func__,
                   !!abstime, etos(rutex_wait_return_ewouldblock));
...

        {
            ...
            switch (state) {
                case RutexWaiter::waiter_state_interrupted:
                    RS_DBG(dbg_rutex,
                           "rutex=%ld | %s | abstime=%d rw=%ld | state eintr in locked | %s", id(),
                           __func__, !!abstime, rw.id(), etos(rutex_wait_return_eintr));
                    return rutex_wait_return_eintr;

                case RutexWaiter::waiter_state_ready:
                    RS_DBG(dbg_rutex,
                           "rutex=%ld | %s | abstime=%d rw=%ld | state ready in locked | %s", id(),
        ...
        RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d | rw=%ld | sleep", id(), __func__, !!abstime,
               rw.id());
        rw.sleep(abstime);

        RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d | rw=%ld | waked | begin join", id(),
        ...
        switch (state) {
            ...
            case RutexWaiter::waiter_state_interrupted:
                RS_DBG(dbg_rutex,
                       "rutex=%ld | %s | abstime=%d rw=%ld | joined state none or intr | %s", id(),
                       __func__, !!abstime, rw.id(), etos(rutex_wait_return_eintr));
                return rutex_wait_return_eintr;
        ...
        RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d rw=%ld | joined state ready | %s", id(),
               __func__, !!abstime, rw.id(), etos(rutex_wait_return_success));
        return rutex_wait_return_success;
    }
    ...
    int notify_one() {
        for (;;) {
            ...
            if (!rw) {
                RS_DBG(dbg_rutex, "rutex=%ld | %s | empty list | return 0", id(), __func__);
                return 0;
            }
            ...
            RS_DBG(dbg_rutex, "rutex=%ld | %s | rw=%ld try_lock=%d", id(), __func__, rw->id(),
                   isLock);
            ...
            lock.unlock();
            if (rw->wake()) {
                RS_DBG(dbg_rutex, "rutex=%ld | %s | rw=%ld wake success | return 1", id(), __func__,
                       rw->id());
                return 1;
            } else {
                RS_DBG(dbg_rutex, "rutex=%ld | %s | rw=%ld wake failed", id(), __func__, rw->id());
            }
        }
    }
    ...
    int requeue(OtherRutex *other) {
        RS_DBG(dbg_rutex, "rutex=%ld | %s | other=%ld", id(), __func__, other->id());
        ...
        for (;; n++) {
            RutexWaiter *rw = static_cast<RutexWaiter *>(waiters_.front());
            if (!rw) {
                RS_DBG(dbg_rutex, "rutex=%ld | %s | other=%ld | done n=%d", id(), __func__,
                       other->id(), n);
                return n;
            }

            RS_DBG(dbg_rutex, "rutex=%ld | %s | other=%ld | rw=%ld", id(), __func__, other->id(),
                   rw->id());
