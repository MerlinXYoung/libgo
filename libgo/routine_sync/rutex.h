...
template <typename IntValueType, bool Reference> struct IntValue;

template <typename IntValueType> struct IntValue<IntValueType, true> {
public:
  inline std::atomic<IntValueType> *value() { return ptr_; }
  inline std::atomic<IntValueType> const *value() const { return ptr_; }
...
template <typename IntValueType> struct IntValue<IntValueType, false> {
public:
  inline std::atomic<IntValueType> *value() { return &value_; }
  inline std::atomic<IntValueType> const *value() const { return &value_; }
...
#define __SWITCH_CASE_ETOS(x)                                                  \
  case x:                                                                      \
    ...
    default:
      return "Unknown rutex_wait_return";
    }
...
public:
  ...
  void mark() { switcher_->mark(); }

  template <typename _Clock, typename _Duration>
  void sleep(const std::chrono::time_point<_Clock, _Duration> *abstime) {
    if (abstime) {
      hasAbsTime_ = true;
      RoutineSyncTimer::getInstance().schedule(
          timerId_, *abstime, [this] { this->wake_by_timer(); });
    }
  ...
  void wake_by_timer() {
    ...
    if (!lock.try_lock()) {
      RS_DBG(dbg_rutex, "rw=%ld | %s | try_lock=0", id(), __func__);
      return;
    }

    if (!safe_unlink()) {
      RS_DBG(dbg_rutex, "rw=%ld | %s | safe_unlink failed", id(), __func__);
      return;
    }

    if (wake(waiter_state_timeout)) {
      RS_DBG(dbg_rutex, "rw=%ld | %s | wake success", id(), __func__);
      return;
    }

    if (!waked_.load(std::memory_order_relaxed)) {
      RS_DBG(dbg_rutex, "rw=%ld | %s | wake failed, retry delay=%d", id(),
             __func__, (int)delayMs_);
      // 可能还没来得及sleep, 往后等一等
      RoutineSyncTimer::getInstance().reschedule(
          timerId_,
          RoutineSyncTimer::now() + std::chrono::milliseconds(delayMs_));
      delayMs_ = delayMs_ << 1;
    } else {
      RS_DBG(dbg_rutex, "rw=%ld | %s | wake failed, needn't retry.", id(),
             __func__);
    }
  }
  ...
  void join() {
    ...
    //   b-1) waitMtx::try_lock成功, 等待执行wake或已执行wake,
    //   需要等待notify完成 b-2) waitMtx::try_lock失败, 不会再执行wake,
    //   无须等待notify完成
    // c) 没上锁，直接失败：同b
    safe_unlink();
...
struct Rutex : public IntValue<IntValueType, Reference>,
               public RutexBase,
               public DebuggerId<RutexBase> {
public:
  ...
  template <typename _Clock, typename _Duration>
  rutex_wait_return
  wait_until(IntValueType expectValue,
             const std::chrono::time_point<_Clock, _Duration> *abstime) {
    if (base_t::value()->load(std::memory_order_relaxed) != expectValue) {
      std::atomic_thread_fence(std::memory_order_acquire);
      RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d | check fail | %s", id(),
             __func__, !!abstime, etos(rutex_wait_return_ewouldblock));
    ...
    if (!switcher->valid()) {
      ...
      if (RoutineSyncPolicy::isInPThread()) {
        ...
        RS_DBG(dbg_rutex,
               "rutex=%ld | %s | abstime=%d | switcher invalid && not in "
               "pthread | %s",
...

    {
      ...
      if (base_t::value()->load(std::memory_order_relaxed) != expectValue) {
        RS_DBG(dbg_rutex,
               "rutex=%ld | %s | abstime=%d rw=%ld | check fail in locked | %s",
               id(), __func__, !!abstime, rw.id(),
               etos(rutex_wait_return_ewouldblock));
        return rutex_wait_return_ewouldblock;
      }
      ...
      case RutexWaiter::waiter_state_interrupted:
        RS_DBG(
            dbg_rutex,
            "rutex=%ld | %s | abstime=%d rw=%ld | state eintr in locked | %s",
      ...
      case RutexWaiter::waiter_state_ready:
        RS_DBG(
            dbg_rutex,
            "rutex=%ld | %s | abstime=%d rw=%ld | state ready in locked | %s",
            id(), __func__, !!abstime, rw.id(),
            etos(rutex_wait_return_success));
    ...
    RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d | rw=%ld | sleep", id(),
           __func__, !!abstime, rw.id());
    rw.sleep(abstime);

    RS_DBG(dbg_rutex,
           "rutex=%ld | %s | abstime=%d | rw=%ld | waked | begin join", id(),
           __func__, !!abstime, rw.id());
    rw.join();
    ...
    case RutexWaiter::waiter_state_interrupted:
      RS_DBG(
          dbg_rutex,
          "rutex=%ld | %s | abstime=%d rw=%ld | joined state none or intr | %s",
          id(), __func__, !!abstime, rw.id(), etos(rutex_wait_return_eintr));
      return rutex_wait_return_eintr;

    case RutexWaiter::waiter_state_timeout:
      RS_DBG(dbg_rutex,
             "rutex=%ld | %s | abstime=%d rw=%ld | joined state timeout | %s",
             id(), __func__, !!abstime, rw.id(),
             etos(rutex_wait_return_etimeout));
      return rutex_wait_return_etimeout;
    }

    RS_DBG(dbg_rutex,
           "rutex=%ld | %s | abstime=%d rw=%ld | joined state ready | %s", id(),
           __func__, !!abstime, rw.id(), etos(rutex_wait_return_success));
    return rutex_wait_return_success;
  }
  ...
  int notify_one() {
    for (;;) {
      ...
      if (!rw) {
        RS_DBG(dbg_rutex, "rutex=%ld | %s | empty list | return 0", id(),
               __func__);
        return 0;
      }

      // 先lock，后unlink, 和join里面的顺序形成ABBA,
      // 确保join可以等到这个函数结束 
      std::unique_lock<std::mutex> lock2(rw->wakeMtx_, std::defer_lock);
      bool isLock = lock2.try_lock();

      RS_DBG(dbg_rutex, "rutex=%ld | %s | rw=%ld try_lock=%d", id(), __func__,
             rw->id(), isLock);
      ...
      if (rw->wake()) {
        ...
        RS_DBG(dbg_rutex, "rutex=%ld | %s | rw=%ld wake failed", id(), __func__,
               rw->id());
      }
    }
  }
  ...
  template <typename OtherRutex> int requeue(OtherRutex *other) {
    RS_DBG(dbg_rutex, "rutex=%ld | %s | other=%ld", id(), __func__,
           other->id());
    ...
    for (;; n++) {
      RutexWaiter *rw = static_cast<RutexWaiter *>(waiters_.front());
      if (!rw) {
        RS_DBG(dbg_rutex, "rutex=%ld | %s | other=%ld | done n=%d", id(),
               __func__, other->id(), n);
        return n;
      }

      RS_DBG(dbg_rutex, "rutex=%ld | %s | other=%ld | rw=%ld", id(), __func__,
             other->id(), rw->id());
