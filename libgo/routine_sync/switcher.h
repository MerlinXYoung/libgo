...
public:
  virtual ~RoutineSwitcherI() { valid_ = false; }
...
public:
  ...
  static bool isInRoutine() noexcept { return true; }
...
private:
  ...
  template <typename S1> inline static RoutineSwitcherI &clsRef_T() {
    if (S1::isInRoutine()) {
      return S1::clsRef();
    }
  ...
  template <typename S1> inline static bool isInPThread_T() {
    if (S1::isInRoutine()) {
      return false;
    }
