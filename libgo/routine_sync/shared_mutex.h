...
struct SharedMutex : public DebuggerId<SharedMutex> {
  public:
    ...
    void unlock() {
        ...
        } while (!whole()->compare_exchange_strong(expect, v, std::memory_order_release,
    ...
    bool try_lock_shared() {
        ...
        uint64_t expect = v;
        internal->locked++;
        bool succ = whole()->compare_exchange_strong(expect, v, std::memory_order_acquire,
  ...
  public:
    ...
    bool lock(const std::chrono::time_point<_Clock, _Duration> *abstime) {
        RS_DBG(dbg_mutex, "shared_mutex=%ld(r=%ld w=%ld) | %s | abstime=%d | enter", id(),
               readers_.id(), writers_.id(), __func__, !!abstime);
        ...
        for (;;) {
            ...
            RS_DBG(dbg_mutex,
                   ...
                   id(), readers_.id(), writers_.id(), __func__, !!abstime, (int)internal->locked,
                   (int)internal->write_wait, (int)internal->read_wait);

            if (0 == internal->locked || -2 == internal->locked) {
                uint64_t expect = v;
                internal->locked = -1;
                if (!whole()->compare_exchange_strong(expect, v, std::memory_order_release,
                                                      std::memory_order_acquire)) {
                    RS_DBG(dbg_mutex,
                           "shared_mutex=%ld(r=%ld w=%ld) | %s | abstime=%d |"
    ...
    void notify(uint64_t &v, FastSharedMutexInternal *internal, const char *funcName) {
        ...
        if (internal->read_wait && internal->locked >= 0 && internal->write_wait == 0) {
            ...
            RS_DBG(dbg_mutex,
                   ...
                   id(), readers_.id(), writers_.id(), funcName, (int)internal->locked,
                   (int)internal->write_wait, (int)internal->read_wait, nr);
            return;
        }
