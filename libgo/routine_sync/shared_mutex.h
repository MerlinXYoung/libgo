...
public:
  SharedMutex() noexcept { readers_.ref(writers_.value()); }
  ...
  // ---------------- write lock
  void lock() { lock(RoutineSyncTimer::null_tp()); }
  ...
  bool try_lock() {
    ...
    if (expect != 0 && expect != -2) {
      RS_DBG(dbg_mutex,
             "shared_mutex=%ld(r=%ld w=%ld) | %s |"
             " internal {locked=%d} | is locked | fail",
             id(), readers_.id(), writers_.id(), __func__, expect);
      return false;
    }

    bool succ = atomicInternal->locked.compare_exchange_strong(
  ...
  bool is_lock() const {
    SharedMutexInternal const *internal = (SharedMutexInternal const *)whole();
  ...
  void unlock() {
    ...
    do {
      ...
      if (-1 != internal->locked) {
        ...
        RS_DBG(dbg_mutex,
               ...
               (int)internal->locked, (int)internal->write_wait,
               (int)internal->read_wait);
        return;
      }
    ...
    } while (!whole()->compare_exchange_strong(
        expect, v, std::memory_order_release, std::memory_order_acquire));
  ...
  bool
  try_lock_until(const std::chrono::time_point<_Clock, _Duration> &abstime) {
    return lock(&abstime);
  }
  ...
  // ---------------- read lock
  void lock_shared() { lock_shared(RoutineSyncTimer::null_tp()); }
  ...
  bool try_lock_shared() {
    ...
    uint64_t expect = v;
    internal->locked++;
    bool succ = whole()->compare_exchange_strong(
        expect, v, std::memory_order_acquire, std::memory_order_relaxed);
    if (succ) {
      RS_DBG(dbg_mutex,
             "shared_mutex=%ld(r=%ld w=%ld) | %s |"
    ...
    do {
      ...
      if (internal->locked <= 0) {
        ...
        RS_DBG(
            ...
            id(), readers_.id(), writers_.id(), __func__, (int)internal->locked,
            (int)internal->write_wait, (int)internal->read_wait);
        return;
      }
...
public:
  ...
  bool lock(const std::chrono::time_point<_Clock, _Duration> *abstime) {
    ...
    for (;;) {
      ...
      RS_DBG(dbg_mutex,
             "shared_mutex=%ld(r=%ld w=%ld) | %s | abstime=%d |"
      ...
      RS_DBG(
          dbg_mutex,
          "shared_mutex=%ld(r=%ld w=%ld) | %s | abstime=%d | begin wait_until",
          id(), readers_.id(), writers_.id(), __func__, !!abstime);

      auto res = writers_.wait_until(v, abstime);
      if (res == RutexBase::rutex_wait_return_etimeout) {
        RS_DBG(dbg_mutex,
               "shared_mutex=%ld(r=%ld w=%ld) | %s | abstime=%d | wait_until "
               "return timeout",
               id(), readers_.id(), writers_.id(), __func__, !!abstime);
        return false;
      }
  ...
  void notify(uint64_t &v, FastSharedMutexInternal *internal,
              const char *funcName) {
    // 执行notify函数之前处于解锁状态,
    // 可能会有其他线程并发执行lock/unlock等函数, 因此要求notify函数可并发执行.
    //
    // 幸运的是, 处于等待状态的writer和reader都可以被无效唤醒,
    // 而不会导致逻辑出错.
    //
    ...
    if (internal->write_wait) {
      ...
      do {
        ...
        if (internal->locked != -2) {
          // 此时有并发, 因为上文设置过locked == -2,
          // 所以其他线程并发的第一个接口必然是lock/try_lock.
        ...
        // 即使locked == -2, 此处依然可能有并发 (其他线程lock后,
        // 处于unlock后的notify流程中) 此处设置的locked = 0,
        // 可能导致其他线程唤醒的writer lock失败(被其他reader抢占),
        // 重走竞争流程, 但这无伤大雅.
    ...
    // 2.尝试唤醒reader
    if (internal->read_wait && internal->locked >= 0 &&
        internal->write_wait == 0) {
      uint64_t expect;
      do {
        v = whole()->load(std::memory_order_relaxed);
        if (internal->locked < 0) {
          ...
          RS_DBG(dbg_mutex,
                 "shared_mutex=%ld(r=%ld w=%ld) | %s |"
                 " internal {locked=%d,w=%d,r=%d} | other thread locked | stop "
                 "notify reader",
                 id(), readers_.id(), writers_.id(), funcName,
                 (int)internal->locked, (int)internal->write_wait,
                 (int)internal->read_wait);
        ...
        if (internal->write_wait) { // 写优先
          ...
          RS_DBG(dbg_mutex,
                 "shared_mutex=%ld(r=%ld w=%ld) | %s |"
                 " internal {locked=%d,w=%d,r=%d} | other thread writer "
                 "waiting | stop notify reader",
                 id(), readers_.id(), writers_.id(), funcName,
                 (int)internal->locked, (int)internal->write_wait,
                 (int)internal->read_wait);
          return;
    ...
    RS_DBG(dbg_mutex,
           ...
           id(), readers_.id(), writers_.id(), funcName, (int)internal->locked,
           (int)internal->write_wait, (int)internal->read_wait);
  }

private:
  inline std::atomic<uint64_t> *whole() { return readers_.value(); }
  inline std::atomic<uint64_t> const *whole() const { return readers_.value(); }
