...
#define LIBGO_ROUTINE_SYNC_MUTEX_CONTENDED                                     \
  (*(const unsigned *)&::libgo::k_mutex_contended_raw)
#define LIBGO_ROUTINE_SYNC_MUTEX_LOCKED                                        \
  (*(const unsigned *)&::libgo::k_mutex_locked_raw)
...
public:
  ...
  bool is_lock() const {
    MutexInternal const *split = (MutexInternal const *)rutex_.value();
    return split->locked == 1;
  }

  void unlock() {
    RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s", id(), rutex_.id(), __func__);
    ...
    if (prev == LIBGO_ROUTINE_SYNC_MUTEX_LOCKED) {
      RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | not contended", id(),
             rutex_.id(), __func__);
      return;
    }
    ...
    (void)res;
    RS_DBG(dbg_mutex,
           "mutex=%ld rutex=%ld | %s | contended | notify_one return %d", id(),
           rutex_.id(), __func__, res);
  }
...
public:
  ...
  bool lock(const std::chrono::time_point<_Clock, _Duration> *abstime) {
    if (try_lock()) {
      RS_DBG(dbg_mutex,
             "mutex=%ld rutex=%ld | %s | abstime=%d | try_lock success", id(),
             rutex_.id(), __func__, !!abstime);
      return true;
    }

    int res = lock_contended(abstime);
    RS_DBG(dbg_mutex,
           "mutex=%ld rutex=%ld | %s | abstime=%d | lock_contended %s", id(),
           rutex_.id(), __func__, !!abstime,
  ...
  template <typename _Clock, typename _Duration>
  inline int
  lock_contended(const std::chrono::time_point<_Clock, _Duration> *abstime) {
    std::atomic<unsigned> *whole = rutex_.value();
    while (whole->exchange(LIBGO_ROUTINE_SYNC_MUTEX_CONTENDED) &
           LIBGO_ROUTINE_SYNC_MUTEX_LOCKED) {
      RS_DBG(dbg_mutex,
             "mutex=%ld rutex=%ld | %s | abstime=%d | begin wait_until", id(),
             rutex_.id(), __func__, !!abstime);

      int res = rutex_.wait_until(LIBGO_ROUTINE_SYNC_MUTEX_CONTENDED, abstime);

      RS_DBG(dbg_mutex,
             "mutex=%ld rutex=%ld | %s | abstime=%d | wait_until return %d",
  ...
  template <typename RutexT> int requeue_in_lock(RutexT *src) {
