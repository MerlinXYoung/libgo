...
struct Mutex : public DebuggerId<Mutex> {
  public:
    ...
    void unlock() {
        ...
        if (prev == LIBGO_ROUTINE_SYNC_MUTEX_LOCKED) {
            RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | not contended", id(), rutex_.id(),
                   __func__);
            return;
        }
        ...
        (void)res;
        RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | contended | notify_one return %d", id(),
  ...
  public:
    ...
    bool lock(const std::chrono::time_point<_Clock, _Duration> *abstime) {
        if (try_lock()) {
            RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | abstime=%d | try_lock success", id(),
                   rutex_.id(), __func__, !!abstime);
            return true;
        }

        int res = lock_contended(abstime);
        RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | abstime=%d | lock_contended %s", id(),
               rutex_.id(), __func__, !!abstime,
    ...
    inline int lock_contended(const std::chrono::time_point<_Clock, _Duration> *abstime) {
        ...
        while (whole->exchange(LIBGO_ROUTINE_SYNC_MUTEX_CONTENDED) &
            ...
            RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | abstime=%d | begin wait_until", id(),
                   rutex_.id(), __func__, !!abstime);

            int res = rutex_.wait_until(LIBGO_ROUTINE_SYNC_MUTEX_CONTENDED, abstime);

            RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | abstime=%d | wait_until return %d", id(),
                   rutex_.id(), __func__, !!abstime, res);
    ...
    RutexType *native() noexcept { return &rutex_; }
