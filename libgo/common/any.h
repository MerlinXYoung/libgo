...
class any {
  ...
  public:  // modifiers
    ...
    typename std::remove_cv<typename std::decay<const ValueType>::type>::type& emplace(
        Args&&... args) {
        clear();
        auto placeholder =
            new holder<typename std::remove_cv<typename std::decay<const ValueType>::type>::type>(
...
ValueType* any_cast(any* operand) noexcept {
    return operand && (operand->type() == typeid(ValueType))
               ? std::addressof(static_cast<any::holder<typename std::remove_cv<ValueType>::type>*>(
                                    operand->content)
                                    ->held)
               : 0;
}
...
#ifdef BOOST_MSVC
#pragma warning(push)
#pragma warning(disable : 4172)  // "returning address of local variable or temporary" but *result
                                 // is not local!
#endif
