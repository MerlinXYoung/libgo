...
struct BooleanFakeLock {
  ...
  ALWAYS_INLINE bool is_lock() const noexcept { return locked_; }
...
struct LFLock {
  ...
  ALWAYS_INLINE void unlock() { flag.clear(std::memory_order_release); }
};

struct FakeLock {
  void lock() {}
  bool is_lock() const noexcept { return false; }
...
struct LFLock2 {
  ...
  ALWAYS_INLINE bool is_lock() const noexcept { return state; }
};
