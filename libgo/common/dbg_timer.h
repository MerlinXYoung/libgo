...
struct DbgTimer {
  ...
  template <typename Duration> void check(int line, Duration precision) {
    if (!active_)
      return;

    auto now = FastSteadyClock::now();
    if (now - prev_ > precision) {
      costs_.push_back(std::pair<int, int>(
          line,
          std::chrono::duration_cast<std::chrono::microseconds>(now - prev_)
              .count()));
    }
    prev_ = now;
  }
  ...
  std::string ToString() const {
    ...
    for (auto &kv : costs_) {
      int len =
          snprintf(buf, sizeof(buf) - 1, "(L=%d, %d us)", kv.first, kv.second);
      s.append(buf, len);
    }
    return s;
  }
