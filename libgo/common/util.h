...
struct fake_lock_guard {
  template <typename Mutex> explicit fake_lock_guard(Mutex &) {}
};
...
struct RefObject {
  ...
  bool IsShared() const noexcept { return reference_ != &referenceImpl_; }

  void IncrementRef() { ++*reference_; }
  ...
  long use_count() const noexcept { return *reference_; }

  void SetDeleter(Deleter d) { deleter_ = d; }
...
public:
  ...
  long use_count() const noexcept { return ptr_ ? (long)*ptr_->reference_ : 0; }

  bool unique() const noexcept { return use_count() == 1; }

  void swap(IncursivePtr &other) { std::swap(ptr_, other.ptr_); }
...
public:
  ...
  explicit WeakPtr(T *ptr) : impl_(nullptr), ptr_(nullptr) { reset(ptr); }
  explicit WeakPtr(IncursivePtr<T> const &iptr)
      : impl_(nullptr), ptr_(nullptr) {
    T *ptr = iptr.get();
    reset(ptr);
  }
  ...
  WeakPtr(WeakPtr &&other) : impl_(nullptr), ptr_(nullptr) { swap(other); }
...
template <typename T> struct ObjectCounter {
  ...
  static long getCount() { return counter(); }
...
// ID
template <typename T> struct IdCounter {
...
struct SourceLocation {
  ...
  void Init(const char *file, int lineno) { file_ = file, lineno_ = lineno; }
