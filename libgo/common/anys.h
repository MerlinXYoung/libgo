...
template <typename Group> class Anys {
public:
  typedef void (*Constructor)(void *);
  typedef void (*Destructor)(void *);

  template <typename T> struct DefaultConstructorDestructor {
  ...
  template <typename T> static std::size_t Register() {
  ...
  static std::size_t Register(Constructor constructor, Destructor destructor) {
    ...
    if (!inited.try_lock())
      throw std::logic_error(
          "Anys::Register mustbe at front of new first instance.");
  ...
  template <typename T> ALWAYS_INLINE T &get(std::size_t index) {
  ...
  template <typename T> ALWAYS_INLINE const T &get(std::size_t index) const {
    if (index >= *(std::size_t *)hold_)
      throw std::logic_error("Anys::get overflow");

    const char *p = storage_ + offsets_[index];
