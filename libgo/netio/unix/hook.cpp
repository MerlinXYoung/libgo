...
#if defined(LIBGO_SYS_Linux)
int libgo_epoll_wait(int epfd, struct epoll_event *events, int maxevents,
                     int timeout) {
...
inline int libgo_poll(struct pollfd *fds, nfds_t nfds, int timeout,
                      bool nonblocking_check) {
  Task *tk = Processer::GetCurrentTask();
  DebugPrint(dbg_hook,
             "task(%s) hook libgo_poll(first-fd=%d, nfds=%d, timeout=%d, "
             "nonblocking=%d). %s coroutine.",
             tk->DebugInfo(), nfds > 0 ? fds[0].fd : -1, (int)nfds, timeout,
             nonblocking_check, Processer::IsCoroutine() ? "In" : "Not in");
  ...
  std::shared_ptr<short int> revents(arrRevents,
                                     [](short int *p) { delete[] p; });
  ...
  for (nfds_t i = 0; i < nfds; ++i) {
    ...
    if (!Reactor::Select(pfd.fd).Add(pfd.fd, pfd.events,
                                     Reactor::Entry(entry, revents, i))) {
      // bad file descriptor
...
static ssize_t read_write_mode(int fd, OriginF fn, const char *hook_fn_name,
                               short int event, int timeout_so, ssize_t buflen,
                               Args &&...args) {
  Task *tk = Processer::GetCurrentTask();
  ...
  int pollTimeout = (socketTimeout == 0)
                        ? -1
                        : (socketTimeout < 1000 ? 1 : socketTimeout / 1000);
...
int socket(int domain, int type, int protocol) {
  ...
  if (sock >= 0) {
    HookHelper::getInstance().OnCreate(sock, eFdType::eSocket, false,
                                       SocketAttribute(domain, type, protocol));
  }

  DebugPrint(dbg_hook, "task(%s) hook socket, returns %d.", tk->DebugInfo(),
             sock);
  return sock;
}
...
int socketpair(int domain, int type, int protocol, int sv[2]) {
  ...
  if (res == 0) {
    HookHelper::getInstance().OnCreate(sv[0], eFdType::eSocket, false,
                                       SocketAttribute(domain, type, protocol));
    HookHelper::getInstance().OnCreate(sv[1], eFdType::eSocket, false,
                                       SocketAttribute(domain, type, protocol));
  }
  return res;
}
...
int connect(int fd, const struct sockaddr *addr, socklen_t addrlen) {
  ...
  Task *tk = Processer::GetCurrentTask();
  DebugPrint(dbg_hook, "task(%s) hook connect. %s coroutine.", tk->DebugInfo(),
             Processer::IsCoroutine() ? "In" : "Not in");
  ...
  if (res == 0) {
    DebugPrint(dbg_hook,
               "continue task(%s) connect completed immediately. fd=%d",
               tk->DebugInfo(), fd);
    return 0;
...
int accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
  ...
  if (!ctx) {
    Task *tk = Processer::GetCurrentTask();
    DebugPrint(dbg_hook, "task(%s) hook accept(fd=%d) no fd_context.",
               tk->DebugInfo(), sockfd);
    errno = EBADF;
    return -1;
  }

  int sock = read_write_mode(sockfd, accept_f, "accept", POLLIN, SO_RCVTIMEO, 0,
                             addr, addrlen);
  if (sock >= 0) {
    HookHelper::getInstance().OnCreate(sock, eFdType::eSocket, false,
                                       ctx->GetSocketAttribute());
  }
  return sock;
}
...
ssize_t read(int fd, void *buf, size_t count) {
  if (!read_f)
    initHook();
  return read_write_mode(fd, read_f, "read", POLLIN, SO_RCVTIMEO, count, buf,
                         count);
}
...
ssize_t readv(int fd, const struct iovec *iov, int iovcnt) {
  ...
  return read_write_mode(fd, readv_f, "readv", POLLIN, SO_RCVTIMEO, buflen, iov,
                         iovcnt);
}
...
ssize_t recv(int sockfd, void *buf, size_t len, int flags) {
  if (!recv_f)
    initHook();
  return read_write_mode(sockfd, recv_f, "recv", POLLIN, SO_RCVTIMEO, len, buf,
                         len, flags);
}
...
ssize_t recvfrom(int sockfd, void *buf, size_t len, int flags,
  ...
  if (!recvfrom_f)
    initHook();
  return read_write_mode(sockfd, recvfrom_f, "recvfrom", POLLIN, SO_RCVTIMEO,
                         len, buf, len, flags, src_addr, addrlen);
}
...
ssize_t recvmsg(int sockfd, struct msghdr *msg, int flags) {
  ...
  return read_write_mode(sockfd, recvmsg_f, "recvmsg", POLLIN, SO_RCVTIMEO,
                         buflen, msg, flags);
}
...
ssize_t write(int fd, const void *buf, size_t count) {
  if (!write_f)
    initHook();
  return read_write_mode(fd, write_f, "write", POLLOUT, SO_SNDTIMEO, count, buf,
                         count);
}
...
ssize_t writev(int fd, const struct iovec *iov, int iovcnt) {
  ...
  for (int i = 0; i < iovcnt; ++i)
    buflen += iov[i].iov_len;
  return read_write_mode(fd, writev_f, "writev", POLLOUT, SO_SNDTIMEO, buflen,
                         iov, iovcnt);
}
...
ssize_t send(int sockfd, const void *buf, size_t len, int flags) {
  if (!send_f)
    initHook();
  return read_write_mode(sockfd, send_f, "send", POLLOUT, SO_SNDTIMEO, len, buf,
                         len, flags);
}
...
ssize_t sendto(int sockfd, const void *buf, size_t len, int flags,
  ...
  if (!sendto_f)
    initHook();
  return read_write_mode(sockfd, sendto_f, "sendto", POLLOUT, SO_SNDTIMEO, len,
...
ssize_t sendmsg(int sockfd, const struct msghdr *msg, int flags) {
  ...
  for (size_t i = 0; i < msg->msg_iovlen; ++i)
    buflen += msg->msg_iov[i].iov_len;
  return read_write_mode(sockfd, sendmsg_f, "sendmsg", POLLOUT, SO_SNDTIMEO,
                         buflen, msg, flags);
}
...
int poll(struct pollfd *fds, nfds_t nfds, int timeout) {
  ...
  DebugPrint(
      dbg_hook,
      "task(%s) hook poll(first-fd=%d, nfds=%d, timeout=%d). %s coroutine.",
      tk->DebugInfo(), nfds > 0 ? fds[0].fd : -1, (int)nfds, timeout,
      Processer::IsCoroutine() ? "In" : "Not in");
...
int __poll(struct pollfd *fds, nfds_t nfds, int timeout) {
  ...
  DebugPrint(
      dbg_hook,
      "task(%s) hook __poll(first-fd=%d, nfds=%d, timeout=%d). %s coroutine.",
      tk->DebugInfo(), nfds > 0 ? fds[0].fd : -1, (int)nfds, timeout,
      Processer::IsCoroutine() ? "In" : "Not in");
...
struct hostent *gethostbyname(const char *name) {
  Task *tk = Processer::GetCurrentTask();
  DebugPrint(dbg_hook, "task(%s) hook gethostbyname(name=%s).", tk->DebugInfo(),
             name ? name : "");
  ...
  int ret = -1;
  while ((ret = gethostbyname_r(name, host, &buf[0], buf.size(), &result,
                                &host_errno)) == ERANGE &&
...
int gethostbyname_r(const char *__restrict name,
  ...
  return gethostbyname_r_f(name, __result_buf, __buf, __buflen, __result,
                           __h_errnop);
}
...
struct hostent *gethostbyname2(const char *name, int af) {
  ...
  int ret = -1;
  while ((ret = gethostbyname2_r(name, af, host, &buf[0], buf.size(), &result,
                                 &host_errno)) == ERANGE &&
         host_errno == NETDB_INTERNAL) {
    if (buf.size() < 1024)
      buf.resize(1024);
...
int gethostbyname2_r(const char *name, int af, struct hostent *ret, char *buf,
  ...
  if (!gethostbyname2_r_f)
    initHook();
  Task *tk = Processer::GetCurrentTask();
  DebugPrint(dbg_hook,
             "task(%s) hook gethostbyname2_r(name=%s, af=%d, buflen=%d).",
...
struct hostent *gethostbyaddr(const void *addr, socklen_t len, int type) {
  Task *tk = Processer::GetCurrentTask();
  DebugPrint(dbg_hook, "task(%s) hook gethostbyaddr(type=%d).", tk->DebugInfo(),
             type);
  ...
  int ret = -1;
  while ((ret = gethostbyaddr_r(addr, len, type, host, &buf[0], buf.size(),
                                &result, &host_errno)) == ERANGE &&
         host_errno == NETDB_INTERNAL) {
    if (buf.size() < 1024)
      buf.resize(1024);
    else
...
int select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
  ...
  Task *tk = Processer::GetCurrentTask();
  DebugPrint(dbg_hook,
             "task(%s) hook select(nfds=%d, rd_set=%p, wr_set=%p, er_set=%p, "
             "timeout=%d ms).",
             tk->DebugInfo(), (int)nfds, readfds, writefds, exceptfds,
             timeout_ms);
  ...
  timeval zero_tv = {0, 0};
  int n =
      select_f(nfds, (readfds ? &rfs : nullptr), (writefds ? &wfs : nullptr),
...
int nanosleep(const struct timespec *req, struct timespec *rem) {
  ...
  if (tk) {
    ...
    DebugPrint(dbg_hook,
               "task(%s) hook nanosleep(milliseconds=%d). %s coroutine.",
  ...
  Processer::Suspend(
      std::chrono::nanoseconds(req->tv_sec * 1000000000 + req->tv_nsec));
  Processer::StaticCoYield();
  return 0;
}
...
ATTRIBUTE_WEAK extern int __socketpair(int domain, int type, int protocol,
                                       int sv[2]);
ATTRIBUTE_WEAK extern int __connect(int fd, const struct sockaddr *addr,
                                    socklen_t addrlen);
ATTRIBUTE_WEAK extern ssize_t __read(int fd, void *buf, size_t count);
ATTRIBUTE_WEAK extern ssize_t __readv(int fd, const struct iovec *iov,
                                      int iovcnt);
ATTRIBUTE_WEAK extern ssize_t __recv(int sockfd, void *buf, size_t len,
                                     int flags);
ATTRIBUTE_WEAK extern ssize_t __recvfrom(int sockfd, void *buf, size_t len,
                                         int flags, struct sockaddr *src_addr,
                                         socklen_t *addrlen);
ATTRIBUTE_WEAK extern ssize_t __recvmsg(int sockfd, struct msghdr *msg,
                                        int flags);
ATTRIBUTE_WEAK extern ssize_t __write(int fd, const void *buf, size_t count);
ATTRIBUTE_WEAK extern ssize_t __writev(int fd, const struct iovec *iov,
                                       int iovcnt);
ATTRIBUTE_WEAK extern ssize_t __send(int sockfd, const void *buf, size_t len,
                                     int flags);
ATTRIBUTE_WEAK extern ssize_t __sendto(int sockfd, const void *buf, size_t len,
                                       int flags,
                                       const struct sockaddr *dest_addr,
                                       socklen_t addrlen);
ATTRIBUTE_WEAK extern ssize_t __sendmsg(int sockfd, const struct msghdr *msg,
                                        int flags);
ATTRIBUTE_WEAK extern int __libc_accept(int sockfd, struct sockaddr *addr,
                                        socklen_t *addrlen);
ATTRIBUTE_WEAK extern int __libc_poll(struct pollfd *fds, nfds_t nfds,
                                      int timeout);
...
ATTRIBUTE_WEAK extern int __nanosleep(const struct timespec *req,
                                      struct timespec *rem);
...
#if defined(LIBGO_SYS_Linux)
ATTRIBUTE_WEAK extern int __gethostbyname_r(
...
ATTRIBUTE_WEAK extern int
__gethostbyname2_r(const char *name, int af, struct hostent *ret, char *buf,
                   size_t buflen, struct hostent **result, int *h_errnop);
ATTRIBUTE_WEAK extern int __gethostbyaddr_r(const void *addr, socklen_t len,
                                            int type, struct hostent *ret,
                                            char *buf, size_t buflen,
                                            struct hostent **result,
                                            int *h_errnop);
ATTRIBUTE_WEAK extern int __epoll_wait_nocancel(int epfd,
...
static int doInitHook() {
  ...
  if (connect_f) {
    ...
    gethostbyname2_r_f =
        (gethostbyname2_r_t)dlsym(RTLD_NEXT, "gethostbyname2_r");
  ...
  if (!pipe_f || !socket_f || !socketpair_f || !connect_f || !read_f ||
      !write_f || !readv_f || !writev_f || !send_f || !sendto_f || !sendmsg_f ||
      !accept_f || !poll_f || !select_f || !sleep_f || !usleep_f ||
      !nanosleep_f || !close_f || !fcntl_f || !setsockopt_f || !getsockopt_f ||
      !dup_f || !dup2_f || !fclose_f
#if defined(LIBGO_SYS_Linux)
      || !pipe2_f || !gethostbyname_r_f || !gethostbyname2_r_f ||
      !gethostbyaddr_r_f || !epoll_wait_f
#elif defined(LIBGO_SYS_FreeBSD)
#endif
    ...
    fprintf(
        stderr,
        "Hook syscall failed. Please don't remove libc.a when static-link.\n");
    exit(1);
  }
  return 0;
}
