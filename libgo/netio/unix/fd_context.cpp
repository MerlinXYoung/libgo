...
FdContext::FdContext(int fd, eFdType fdType, bool isNonBlocking, SocketAttribute sockAttr)
    ...
    DebugPrint(dbg_fd_ctx,
               "create FdContext(fd = %d, type = %s, isNonBlocking = %d, attr(%d,%d,%d)", fd,
               FdType2Str(fdType), (int)isNonBlocking, sockAttr_.domain_, sockAttr.type_,
               sockAttr_.protocol_);
}
...
void FdContext::OnSetNonBlocking(bool isNonBlocking) { isNonBlocking_ = isNonBlocking; }
