...
FdContext::FdContext(int fd, eFdType fdType, bool isNonBlocking,
  ...
  DebugPrint(
      dbg_fd_ctx,
      "create FdContext(fd = %d, type = %s, isNonBlocking = %d, attr(%d,%d,%d)",
      fd, FdType2Str(fdType), (int)isNonBlocking, sockAttr_.domain_,
      sockAttr.type_, sockAttr_.protocol_);
}
bool FdContext::IsSocket() const noexcept {
  return fdType_ == eFdType::eSocket;
}
bool FdContext::IsTcpSocket() const noexcept {
  if (!IsSocket())
    return false;
...
bool FdContext::IsNonBlocking() const noexcept { return isNonBlocking_; }
void FdContext::SetTcpConnectTimeout(int milliseconds) {
  tcpConnectTimeout_ = milliseconds;
}
int FdContext::GetTcpConnectTimeout() { return tcpConnectTimeout_; }
