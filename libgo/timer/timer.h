...
class CoTimer {
public:
  typedef ::libgo::RoutineSyncTimerT<::libgo::Mutex, ::libgo::ConditionVariable>
      CoroutineTimer;
  typedef CoroutineTimer::func_type func_t;

  struct TimerIdImpl {
  public:
    explicit TimerIdImpl(std::shared_ptr<CoroutineTimer> const &timer)
        : timer_(timer) {}
...
public:
  ...
  explicit CoTimer(std::chrono::duration<Rep, Period> dur,
  ...
  TimerId ExpireAt(std::chrono::duration<Rep, Period> dur, func_t const &fn) {
    return ExpireAt(std::chrono::duration_cast<FastSteadyClock::duration>(dur),
                    fn);
  }
