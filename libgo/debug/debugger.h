...
public:
  ...
  public:
    static std::shared_ptr<DbgMap> SafeGetDbgMap() {
      GetLock().lock();
      return std::shared_ptr<DbgMap>(&GetDbgMap(),
                                     [](DbgMap *) { GetLock().unlock(); });
    }
  };
#else
  {
...
public:
  ...
  unsigned long GetCurrentTaskID() const;

  // 当前协程切换的次数
  unsigned long GetCurrentTaskYieldCount() const;

  // 设置当前协程调试信息, 打印调试信息时将回显
  void SetCurrentTaskDebugInfo(std::string const &info);

  // 获取当前协程的调试信息, 返回的内容包括用户自定义的信息和协程ID
  const char *GetCurrentTaskDebugInfo() const;
...
template <typename T> struct real_typename_helper {};

template <typename T> std::string real_typename() {
#if defined(__GNUC__)
  /// gcc.
  int s;
  char *realname =
      abi::__cxa_demangle(typeid(real_typename_helper<T>).name(), 0, 0, &s);
