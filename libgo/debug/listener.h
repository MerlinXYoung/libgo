...
public:
  ...
  public:
     ...
     *             ！！注意：当 exception_handle 配置为 immedaitely_throw
     * 时本事件 ！！与 onFinished()
     * 均失效，异常发生时将直接抛出并中断程序的运行，同时生成coredump
     */
    virtual void onException(uint64_t task_id,
                             std::exception_ptr &eptr) noexcept {}

    /**
     * 协程运行完成，if(eptr)
     *为false说明协程正常结束，为true说明协程抛出了了异常 （本方法运行在协程中）
     *
     * @prarm task_id 协程ID
     */
    ...
    //                                           -->[c]onCompleted->
    //                                          |                   |
    // [s]onCreated-->[s]onSwapIn-->[c]onStart->*--->[c]onSwapOut--
    // -->[c]onFinished-->[c]onSwapOut
    //                                          |\                | |
    //                                          | \<-[s]onSwapIn--V |
    //                                          |                   |
    //                                           -->[c]onException->
