...
std::string CoDebugger::GetAllInfo() const {
  std::string s;
  s += P("==============================================");
  ...
  for (auto &kkv : locMap) {
    ...
    for (auto &kv : kkv.second) {
      s += P("    [%d] Loc {%s}", (int)kv.second.size(),
...
int CoDebugger::TaskCount() const {
#if ENABLE_DEBUGGER
...
unsigned long CoDebugger::GetCurrentTaskID() const {
  return g_Scheduler.GetCurrentTaskID();
}
unsigned long CoDebugger::GetCurrentTaskYieldCount() const {
...
const char *CoDebugger::GetCurrentTaskDebugInfo() const {
